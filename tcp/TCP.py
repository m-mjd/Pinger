...
while True:
    ...
    for i in range(number):
        ...
        try:
            ...
            print(Fore.CYAN + f"{i+1}. {hostname}:{port} ping delay = {times[-1]:.5f} MS" + Style.RESET_ALL)
...
{Fore.RED}{max_time:.5f} MS{Style.RESET_ALL}""")
        print()
        print(f"""حداقل زمان پاسخگویی:

{Fore.GREEN}{min_time:.5f} MS{Style.RESET_ALL}""")
        print()
        print(f"""میانگین زمان پاسخگویی:

{Fore.BLUE}{avg_time:.5f} MS{Style.RESET_ALL}""")
        print()
        print()
        print()
